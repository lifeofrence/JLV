...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4><i class="bi-calendar-check-fill"></i> Bookings</h4>
    <span class="text-secondary"><?= $total ?> total</span>
</div>

<?php if (empty($rows)): ?>
    <div class="alert alert-info"><i class="bi-info-circle"></i> No bookings yet.</div>
<?php else: ?>
        ...
        <?php foreach ($rows as $row): ?>
            <div class="entry-card status-<?= $row['status'] ?>">
                ...
                <div class="entry-body">
                    ...
                    <div class="entry-row">
                        <span class="erl">Package</span>
                        <span class="erv"><?= htmlspecialchars($row['package']) ?></span>
                    </div>
                    <div class="entry-row">
                        <span class="erl">Event Date</span>
                        <span class="erv"><?= $row['event_date'] ? date('d-M-Y', strtotime($row['event_date'])) : '-' ?></span>
                    </div>
                    <?php if ($row['message']): ?>
                ...
                <div class="entry-actions">
                    <a href="reply.php?table=bookings&id=<?= $row['id'] ?>" class="btn btn-admin btn-admin-sm"><i class="bi-reply-fill"></i> Reply</a>
                    <?php if ($row['status'] === 'new'): ?>
                        <a href="mark.php?table=bookings&id=<?= $row['id'] ?>&status=replied&page=<?= $page ?>" class="btn btn-sm btn-outline-info">Mark Replied</a>
                    <?php elseif ($row['status'] === 'replied'): ?>
                        <a href="mark.php?table=bookings&id=<?= $row['id'] ?>&status=completed&page=<?= $page ?>" class="btn btn-sm btn-outline-success">Mark Completed</a>
                    <?php endif; ?>
                </div>
            </div>
        <?php endforeach; ?>
